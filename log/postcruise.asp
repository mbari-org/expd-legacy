...
<%
  ...
  <br><br>(Will add a dive to ExpeditionID <%= GetFormValue('ExpeditionID')%>)
...
function ShipSelect(select) {
  ...
  if (select.options[select.selectedIndex].value == "othr") {
    	...
    	if (!sessionStorage.returning_othr) {
    	// SWEETALERT
     	swal({
        html:
          '<br> ' +
      ...
      sessionStorage.returning_othr = true; // set returning
    } // End of if
    return(true)
	}
  else if (select.options[select.selectedIndex].value == "zphr") {
      // Show alert only once.
      if (!sessionStorage.returning_zphr) {
      // SWEETALERT
      swal({
        html:
          '<br> ' +
          '<b>Note:  The MBARI R/V <i>Zephyr</i> operated only from February 2002 - September 2012. If searching by date, make sure to adjust your search to reflect this.</b>',
        showCloseButton: true,
        showCancelButton: false,
        confirmButtonText:
          'Got it!',
      ...
      sessionStorage.returning_zphr = true; // set returning
    } // End of if
    return(true)
  }
  else if (select.options[select.selectedIndex].value == "ptlo") {
      // Show alert only once.
      if (!sessionStorage.returning_ptlo) {
      // SWEETALERT
      swal({
        html:
          '<br> ' +
          '<b>Note:  The MBARI R/V <i>Point Lobos</i> operated only from August 1988 - December 2011. If searching by date, make sure to adjust your search to reflect this.</b>',
        showCloseButton: true,
        showCancelButton: false,
        confirmButtonText:
          'Got it!',
      ...
      sessionStorage.returning_ptlo = true; // set returning
    } // End of if
    return(true)
  }

	if (select.options[select.selectedIndex].value == "dpkd" || "wfly" || "rcsn" || "ptlo" || "zphr" || "prgn" || "bhrzn" || "kilom" || "flmr" || "sirwl" || "ocre1" || "araon") {
...
function ConfigForm() {
	  ...
	  if (selectedShip == "othr") {
	    ...
	    //else if (selectedShip == "dpkd" || "wfly" || "rcsn" || "ptlo" || "zphr" || "prgn") {
      else {
    ...
    <tr>
      ...
      <td>
        ...
        <select id="qShipName" name="qShipName" text-align-last="center" onChange="return ShipSelect(this)">
		      <option value=""><  --Select First (If Known)--></option>		
        <optgroup label="MBARI Ship">
          <option text-align="right" value="dpkd">&emsp;&emsp;&emsp;R/V <i>David Packard</i></option>
          <option text-align="right" value="prgn">&emsp;&emsp;&emsp;R/V <i>Paragon</i></option>
          <option value="ptlo">&emsp;&emsp;&emsp;R/V <i>Point Lobos</i></option>
          <option value="rcsn">&emsp;&emsp;&emsp;R/V <i>Rachel Carson</i></option>  
		  <option value="wfly">&emsp;&emsp;&emsp;R/V <i>Western Flyer</i></option>
		  <option value="zphr">&emsp;&emsp;&emsp;R/V <i>Zephyr</i></option>
        </optgroup>
        <optgroup class="optgroup label" label="Non-MBARI Ship - MiniROV">
          <option value="araon">&emsp;&emsp;&emsp;R/V <i>Araon</i> (KOPRI)</option> 
          <option value="bhrzn">&emsp;&emsp;&emsp;R/V <i>Bold Horizon</i> (CalCOFI)</option>
          <option value="flmr">&emsp;&emsp;&emsp;R/V <i>Fulmar</i> (MBNMS)</option>
          <option value="kilom">&emsp;&emsp;&emsp;R/V <i>Kilo Moana</i> (UH-SOEST)</option>
          <option value="ocre1">&emsp;&emsp;&emsp;R/V <i>Ocean Researcher 1</i> (TORI)</option>
          <option value="sirwl">&emsp;&emsp;&emsp;R/V <i>Sir Wilfrid Laurier</i> (CCGS)</option>
...
####print "<br>PostCruise: $expdDeviceID<br>";
  ...
  if ($Status eq 'Completed') {
    if ( $RS->Fields('ShipName')->Value eq 'bhrzn' || $RS->Fields('ShipName')->Value eq 'kilom' || $RS->Fields('ShipName')->Value eq 'flmr' || $RS->Fields('ShipName')->Value eq 'ocre1' || $RS->Fields('ShipName')->Value eq 'sirwl' || $RS->Fields('ShipName')->Value eq 'araon') {
  ...
  if ( $RovName || $RS->Fields('ShipName')->Value ne 'prgn' || $RS->Fields('ShipName')->Value ne 'zphr') {
      ...
      if ( $rdbdlinks ) {
          ...
          ####print "<br>EXPD Text: " . $expd_text . "<br>";
      }
      elsif ( $proclinks ) {
...
####print "<br>FDLINKS:  $fdlinks<br>";
  ...
  $ndlinks .= $rawNavdbLinks if $rawNavdbLinks;  
  ...
  $ndlinks .=  "<br>" . $cleanNavdbLinks . "<b>&middot;</b>" if $cleanNavdbLinks;
  $ndlinks .=  "" if !$cleanNavdbLinks;
...
<% if ( $Session->{'RegisteredUser'} ) { %> 
  ...
  @listserver.mbari.org.</font></strong></p>
  <br>
  <center>
...
if ($qRovName == 'mini') {
  ...
  %><h2><font color="#408080"><strong>**You are searching on MiniROV dives. If your query resulted in no matches,
    please <br>consider revising your <a href="https://mww.mbari.org/expd/log/postcruise.asp?search=advanced">EXPD search(es)</a> to include both MBARI and Non-MBARI vessels.<br>
    <br><br>****If you feel your search is correct and the MBARI Expedition DataBase (EXPD) does not <br>reflect recent MiniROV data and/or cruises, please contact:<br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;</font><font color="blue">---> Ben Erwin (x-1838, berwin@mbari.org)</font><font color="#408080"><br>to request that newer MiniROV records be added to the MBARI EXPD.</strong></font>
   </h2>
  <%}
} else {
    %>
...
sub email {
  ...
  use Net::SMTP;
    ...
    $smtp = Net::SMTP->new('relay.mbari.org'); 	# connect to an SMTP server
...
sub finished_email {
	...
	$smtp->to( GetFormValue('post_email') . "\@listserver.mbari.org" );		# recipient's address
	$smtp->data();									# Start the mail
	...
	$smtp->datasend("To: " . GetFormValue('post_email') . "\@listserver.mbari.org" . "\n");
	...
	$smtp->quit;                        # Close the SMTP connection
	%>
<h3>The following email has been sent to <%= GetFormValue('post_email')%>@listserver.mbari.org</h3>
	<pre>
<%= $mail_msg%>
	</pre>
<%
