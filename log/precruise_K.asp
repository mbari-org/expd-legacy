...
precruise_K.asp - Application for submitting precruises
...
%>
        ...
        <input type="text"  size=8 name="personal_email" value="salamy"><b>@mbari.org</b>
...
#
  ...
  #use warnings;
    ...
    my $personal_email = "salamy@mbari.org";
    ...
    $smtp->datasend("To: $personal_email\n");
    #$smtp->datasend("From: $personal_email\n");
    ...
    #}
    $smtp->datasend("Subject: Precruise TESTING $Session->{'ScheduledStartDtg'} (Web form $this_script)\n");
    $smtp->datasend("\n");
...
#
    ...
    # Creating a separate var to determine the issues with emails
    my $precruise_email = GetFormValue('pre_email');
    my $formatted_pre_email;      # Declare the variable

    if ($precruise_email) {
        $formatted_pre_email = "$precruise_email\@listserver.mbari.org";
    } else {
        $formatted_pre_email = undef;
    }
    ...
    ####################################################
    my @revised_listserver_recipients = @all_listserver_recipients; 
    ...
    # Send to properly formatted precruise@listserver.mbari.org (pre_email)
    ...
    #$smtp->datasend("To: " . $formatted_pre_email );
    #$smtp->datasend("From: " . $Session->{'logistics_email'} . " (via Web form $this_script)\n");

    if ($is_revised) {
        # Replace all CC recipients with just the revised listserver of all recipients
        #if (@revised_listserver_recipients) {
        #    $smtp->datasend("Cc: " . join(", ", @revised_listserver_recipients) . "\n");
        #}
    } else {
        # Keep the original CC logic for non-revised emails
        #my @cc_recipients = grep { $_ ne (GetFormValue('pre_email') . "\@listserver.mbari.org") } @recipient_emails;
        #if (@cc_recipients) {
        #    $smtp->datasend("Cc: " . join(", ", @cc_recipients) . "\n");
        #}
    }



    $smtp->datasend("Subject: $message Precruise TESTING: $Session->{'ExpdChiefScientist'} $Session->{'ScheduledStartDtg'} GMT\n");
    $smtp->datasend("\n");
