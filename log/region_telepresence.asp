...
$this_script = 'region_telepresence.asp';
#
...
<a href="region_telepresence.asp?add=new">click here to add a region to the list</a>.</b>
<p>
...
<% while ( !$RS->EOF ) { %>
	...
	<% foreach $f (@fields) { 
		...
		if ($f eq 'RegionID') {
			$val = "<a href=\"region_telepresence.asp?edit=$val\">$val</a>";
		} %><%= $cell%><%= $val%></td>
	<% } %>
	<% $RS->MoveNext; %>
<% } %>
...
<form method="get" action="region_telepresence.asp">
...
#<p><a href="mike_precruise_telepresence.asp?step=1">Return to Pre-Cruise entry form step 1</a></p>
<p><a href="precruise_telepresence.asp?step=1">Return to Pre-Cruise entry form step 1</a></p>
<p><a href="region_telepresence.asp">Return to Region List</a></p>
