...
<!--#include file="dive_hdr.inc"-->
<!--#include file="expd_functions.inc"-->

<% 
=head1 NAME

dive.asp - Dive edit/update/delete Active Server Page

=head1 SYNOPSIS

    > http://expd.mbari.org/expd/log/dive.asp
...
######################################################################################################
	...
	$rov = $RS->Fields('RovName')->value || GetFormValue('RovName');
if ($RSt->Fields('ExpeditionID_FK')->value eq 0 && $RSt->Fields('RovName')->value eq 'mini') {
	...
	$sql .= ", DateDiff(\"ss\", '01/01/70', Expedition.ScheduledStartDtg) AS ScheduledStartEpoch " if $ExpdID;
	$sql .= ", DateDiff(\"ss\", '01/01/70', Expedition.ScheduledEndDtg) AS ScheduledEndEpoch " if $ExpdID;
	$sql .= " FROM Dive \n";
	$sql .= "JOIN Expedition ON Dive.ExpeditionID_FK = Expedition.ExpeditionID \n" if $ExpdID;
	$sql .= "WHERE ";
	...
	$sql .= "Dive.DiveNumber = " . $dn if $dn;
	$sql .= " AND Dive.ExpeditionID_FK = " . $ExpdID if $ExpdID;
    $sql .= " AND Dive.RovName = '" . $rov . "'" if $rov;
    $sql .= " AND Expedition.ShipName = '" . $RS->Fields('ShipName')->Value . "'" if $RS->Fields('ShipName')->Value;
    $sql .= " AND Expedition.ShipSeqNum = " . $RS->Fields('ShipSeqNum')->Value if $RS->Fields('ShipSeqNum')->Value;
    ...
    foreach $f ( @{$Application->{'DiveFields'}} ) { 
	    if ( $f eq 'ExpeditionID_FK' ) { 
	    	if ( $RS->Fields('ExpeditionID_FK')->value eq 0 ) {
				...
				<td valign="top"><%= $font_1%><a href="postcruise.asp?step=5&RovName=<%=$RSdives->Fields('RovName')->value%>&DiveNumber=<%=$RSdives->Fields('DiveNumber')->value%>ShipName=<%=$RS->Fields('ShipName')->value%>&edit=<%= $RS->Fields($f)->value%>" target="expd">
	    	...
	    	<%}
				...
				<td valign="top"><%= $font_1%><a href="postcruise.asp?step=3&ShipName=<%=$RS->Fields('ShipName')->value%>&ShipSeqNum=<%=$RS->Fields('ShipSeqNum')->value%>&edit=<%= $RS->Fields($f)->value%>" target="expd">
				<%= $RS->Fields($f)->value%></a> (Click to view this dive's Expedition information in another window)</font></td>
	    	<%}
	    }
	    elsif ( $f eq 'RovName' ) { %>
		...
		foreach ( @logrLinks ) { 
			...
			<a href="javascript:OpenData('/expd/queries/greplogr.asp?url=<%= $_%>&string=DIVE_NUM.*NEW','logrparse')"><%= $logrFile%></a>
		<% } %>
			</font></td>			
	    <% }
...
sub SelectEpoch {
 ...
 ####print "<br>timefield: $time_field,  ExpeditionID: $ExpeditionID<br>";
    Win32::ASP::DebugPrint("\nRSt->Fields('RovName')->value = '" . $RSt->Fields('RovName')->value . "'\n");
    Win32::ASP::DebugPrint("\nrov = '$rov' \n");
...
####$ExpdEndEsecs = SelectEpoch("EndDtg", $ExpdID) || SelectEpoch("ScheduledEndDtg", $ExpdID);
####Win32::ASP::DebugPrint("\nEndEpoch = $EndEpoch");
####print "<br>$StartEpoch, $ExpdStartEsecs, $EndEpoch, $ExpdEndEsecs " . $RSexpd->Fields('ShipName')->Value . "<br>";
...
$rovName = 'docr' if ($RSexpd->Fields('ShipName')->Value eq 'wfly' && $ExpdStartEsecs > 1230768000 && $ExpdStartEsecs < 1672560000);	# after 1/1/2009 and Before 1/1/2023
$rovName = 'docr' if ($RSexpd->Fields('ShipName')->Value eq 'dpkd' && $ExpdStartEsecs > 1672560000);	# after 1/1/2023
$rovName = 'mini' if ($RSexpd->Fields('ShipName')->Value eq 'othr' && ($RSexpd->Fields('DeviceIDD')->Value eq 1859 || $qRovName eq 'mini'));

if ( $RSexpd->Fields('ShipName')->Value eq 'prgn' ) {
	print "<h2><font color=brown>The MBARI R/V <i>Paragon</i> does not deploy ROVs.</font></h2>";
	} 
else {
...
foreach $f ( @{$Application->{'DiveFields'}} ) { 
    ...
    <%}
		...
		$checked_docr = 'checked' if ($RSexpd->Fields('ShipName')->Value eq 'wfly' && $ExpdStartEsecs > 1230768000 && $ExpdStartEsecs < 1672560000);	# after 1/1/2009 and Before 1/1/2023
		$checked_docr = 'checked' if ($RSexpd->Fields('ShipName')->Value eq 'dpkd' && $ExpdStartEsecs > 1672560000);	# after 1/1/2023
		...
		$checked_mini = 'checked' if ($RSexpd->Fields('ShipName')->Value eq 'othr' && $RSdives->Fields('DeviceID')->Value eq 1859 || $RSdives->Fields('DeviceID')->Value eq 1869 || $qRovName eq 'mini');			
		%>
...
if ( keys %$Errors ) { 
	...
	<a href="postcruise.asp?step=2&Conjunction=AND&search=advanced&Continue=Search+for+expeditions&qDiveNumber=<%= GetFormValue('DiveNumber')%>&qRovName=<%= GetFormValue('RovName')%>">
...
sub email_dive_insert {
	...
	if ($_[0] eq 'update' ) {
		...
		$hyperlink = "$baseurl/dive.asp?RovName=" . GetFormValue('RovName') . "&DiveNumber=" . GetFormValue('DiveNumber') .
			"&edit=yes";
	}
	else {
		$mail_msg .= "Click below to edit this postcruise and add the dive. Copy above fields into the web form\n";
		$baseurl = $Application->{'BaseUrl'};
		$hyperlink = "$baseurl/dive.asp?ExpeditionID=" . GetFormValue('ExpeditionID') . 
