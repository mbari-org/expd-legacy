...
if ( GetFormValue('step') eq '1' || ! GetFormValue('step') ) {
  %><h2>Precruise entry: Step 1</h2>
   <p>
   <ul style="width: 1200px; color:#408080; line-height: 0.85; font-weight: bold; font-size: 18px" >
    ...
    <li>To modify an existing cruise plan email requested changes to the <a href="mailto:mschultz@mbari.org">logistics coordinator</a>.</li><br>
    <li><a target="_blank" href="http://www.mbari.org/at-sea/cruise-planning/">DMO cruise planning</a> link.</li><br>
    <li><a target="_blank" href="http://mww.mbari.org/cruises/">MBARI expedition database</a> link.</li></ul></p> 
  </td></tr></table>
   <% 
  step1();
}
...
function joinCruisePurpose(){
  ...
  var dc = document.getElementById('diverRequested').value;  //Yes or No
  //console.log("DC = " + dc);
  ...
  document.getElementById('allCruisePurposeInfo').value = y + " <br>" + z;

  var acp = document.getElementById('allCruisePurposeInfo').value;
  var purposeStr = strip_tags(acp);
...
//////////////////////////
  ...
  if ((equip == undefined || equip.length == 0) && (document.getElementById('EquipmentDesc').value == "")) {
    ...
    a = equipTxt.toString().bold() + v + "<br>" + otherE.toString().bold() + v + "<br>";
  } 
  // Nothing entered in Checkboxes - Text Entered in textarea
    ...
    // Nothing was chosen for equipment
    a = equipTxt.toString().bold() + "CUSTOM" + "<br>" + otherE.toString().bold() + v + "<br>";
  }
  // Something entered in CheckBoxes - Nothing in Textarea
  else if ((equip.length > 0) && (document.getElementById('EquipmentDesc').value == "")) {
    v = "None";
    a = equipTxt.toString().bold() + equip.join(", ") + "<br>" + otherE.toString().bold() + v + "<br>";
  }
...
///////////////////////
  ...
  // Nothing entered in Checkboxes - Text Entered in textarea
    ...
    // Nothing was chosen for equipment
    b = winchTxt.toString().bold() + "CUSTOM" + "<br>" + otherW.toString().bold() + w + "<br>";
  }
...
<!-- SET UP HTML FORM -->
<form method="POST" action="<%=$this_script%>" onsubmit="return Field_Validator(this)" name="Field" id="Form">
<br>
      ...
      <table border="0" cellpadding="5" cellspacing="0"> 
              ...
              <tbody id="cruise_specific_info_section" style="display: none;">
                ...
                <tr id="chief-scientist">
                  ...
                  <td valign="top">
                    ...
                    <a href="mailto:mschultz@mbari.org">Add/edit person to list</a>
                  </td>
                </tr>
          ...
          <div class="padding" id="cruise-purpose-section" style="display:none;">
            ...
            <p class="padding"><strong><font face="Arial,Helvetica" size=5 color=green>I. Cruise purpose</font></strong><br><br>
              <font face="Arial,Helvetica" size="3" color="black"><strong>Enter objective for the expedition</strong></font><font face="Arial,Helvetica" size="-1" color="brown"> (required).</font><br>
              <textarea rows="10" name="Purpose" value="Purpose" id="Purpose" cols="160" wrap></textarea>
            </p>
            <br><br><br>
            ...
            <br><br>
            <input id="allCruisePurposeInfo" type="hidden" name="allCruisePurposeInfo" value="" />
            <input id="allCruisePurposeInfo_cleanHTML" type="hidden" name="allCruisePurposeInfo_cleanHTML" value="" />
          </div>
          ...
          <div id="hazardous-materials-section" style="display:none;">
            <hr>
          <p class="padding">
            <strong><font face="Arial,Helvetica" size=5 color="green">III. Hazardous materials declaration and description</font></strong><br><br>
          ...
          <p class="padding"><strong><font face="Arial,Helvetica" size=5 color="green">IV. Winch requirements</font></strong><br><br>
          <font face="Arial,Helvetica" size="3" color="black"><strong>Check all that apply</strong>.</font><br><br>
          ...
          <textarea class="textfield" input="text" rows="10" id="WinchDesc" name="WinchDesc" value="WinchDesc" cols="160" placeholder="Please enter winch details hereother-equipemtn" onclick='checkWinchOther()' wrap></textarea><br>
          <br><br>
...
<!--             <br><br><br><br><font face="Arial,Helvetica" size="3" color=black><strong>Will you be operating in a <a id="smpa-link" target="_blank" href="https://nrm.dfg.ca.gov/FileHandler.ashx?DocumentID=105423&inline">State Marine Protected Area (SMPA)</a>?</strong><font face="Arial,Helvetica" size="2" color=brown> (Required)
            ...
            href="https://nrm.dfg.ca.gov/FileHandler.ashx?DocumentID=105423&inline">SMPA</a>, do you have a valid permit from the <a id="smpa-cdfw-permit-link" target="_blank" 
...
<!--             <form action="#" onclick="if(document.getElementById('agree').checked) { 
            return true;
            } else { 
            ...
            <font face="Arial,Helvetica" size="3" color=black><strong>If the submitted plan requires changing while offshore this must be discussed with the Captain for review.<br><br>
              <font face="Arial,Helvetica" size="3" color=brown><input type="checkbox" name="MBARIPolicyAgree" value="MBARIPolicyAgree" id="MBARIPolicyAgree" /> I agree to follow <a id="precruise_plans_link" target="_blank">precruise plans</a> and MBARI <a id="mbari_ship_policy_link" target="_blank">policies/procedures</a>.</strong>
            </form>
          </p>
        </div>
        ...
        <div id="submit-button-section" style="display:none;">
          <p></p><br>
          <p class="button"><input type="submit" class="button blue" value="Continue" name="Step2" style="width: 80px;">
            <input type="reset" value="Reset form fields" name="Reset form" style="width: 110px;">
          </p><br><br>
        </div>
    </td>
  </tr>
...
<br>If something is not right here then press your browser's Back button and fix it.
...
<% }
        ...
        cleanHTML();
        %>
        <p class="button"><input type="submit" class="button blue" name="finish" style="width: 200px" value="Email Form to Logistics Coordinator"></p>

<% } %>

        <br>
        <!-- EMAIL ALIASING - A BIT CLUGEY BUT WORKS  -->
        <input type="hidden" name="diver-requested" value="<%=GetFormValue('diverRequested')%>">
        <input type="hidden" name="asv-check" value="<%=GetFormValue('waveG')%>">
        <input type="hidden" name="ctd-check" value="<%=GetFormValue('ctdCheck')%>">
        <input type="hidden" name="hazIso-check" value="<%=GetFormValue('hazIsoCheck')%>">
        <input type="hidden" name="winch-check" value="<%=GetFormValue('winchCheck')%>">
        <input type="hidden" name="itar-check" value="<%=GetFormValue('itarCheck')%>">
        <input type="hidden" name="foreignNational-check" value="<%=GetFormValue('foreignNationalCheck')%>">
        <input type="hidden" name="specialOps-check" value="<%=GetFormValue('specialOpsCheck')%>">
        <input type="hidden" name="permits-check" value="<%=GetFormValue('permitsCheck')%>">
        <br>
</form>
...
#
    ...
    $smtp->to( $Session->{'logistics_email'} );
...
%>
  <script Language="JavaScript">
  //console.log("Entered cleanHTML() method.");
  </script>
<%
    ...
    $Session->{'diver-requested'} = GetFormValue('diverRequested');
    $Session->{'waveG'} = GetFormValue('waveG');
    $Session->{'ctdCheck'} = GetFormValue('ctdCheck');
    $Session->{'hazIsoCheck'} = GetFormValue('hazIsoCheck');
    $Session->{'winchCheck'} = GetFormValue('winchCheck');
...
<%
  ...
  #
  # Save HTML CLEANED step 2 variables for use in email message
  #
