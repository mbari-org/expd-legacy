...
if ( GetFormValue('step') eq '1' || ! GetFormValue('step') ) {
	if (GetFormValue('search') eq 'advanced' ) {
		...
		<strong><font color="#408080">Enter any parameters below to restrict search for expeditions, or use <a href="<%=$this_script%>">keyword search</a>.</font></strong>
...
<%
	...
	<font size="+3">
	<input type="submit" name="add_dive" value="Continue to enter new dive information">
	</font>
...
<table border="0" cellpadding="5" cellspacing="0">
            ...
            <tr>
              ...
              <input type="submit" name="Continue" value="Search expeditions"> &nbsp; <input type="reset" name="Reset form" value="Reset"></td>
            </tr>
            <tr>
              <td align="center"><div align="right"><p><strong><a href="<%= $this_script%>?search=advanced">&gt;&gt;Advanced
              search</a> <br>
              </strong>Search the database for other <br>
              parameters, including date</td>
            </tr>
          </table>
</td></tr>
</table>
...
if(!$RScs) {
 	die "<BR>Empty Return Set: RScs.<br> from $dsn ";
}
...
if(!$RSrgn) {
 	die "<BR>Empty Return Set: RSrgn.<br> from $dsn ";
}
%>
...
<form method="POST" action="<%= $this_script%>" id="form1" name="form1">
  ...
  <table border="0" cellpadding="5" cellspacing="0">
    ...
    <tr>
      ...
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Sequence Number</font></strong></td>
      <td><input type="text" name="qShipSeqNum" size="5">
    ...
    <tr>
      ...
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Status</font></strong></td>
    ...
    <tr>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Purpose</font></strong></td>
      <td><input type="text" name="qPurpose" size="20" value=""></td>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Single Date</font></strong></td>
      <td>
    ...
    <tr>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Site/Track</font></strong></td>
      <td><input type="text" name="qPlannedTrackDesc" size="20" value=""></td>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Participants</font></strong></td>
      <td><input type="text" name="qParticipants" size="20" value=""></td>
    </tr>
    <tr>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Accomplishments</font></strong></td>
      <td><input type="text" name="qAccomplishments" size="20" value=""></td>
    ...
    <tr>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Operator Comments</font></strong></td>
      <td><input type="text" name="qOperatorComments" size="20" value=""></td>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Principal Investigator</font></strong></td>
      <td>
	<select name="qExpdPrincipalInvestigator" size="1">
            <option value=""></option>
	    <% 
	    $RScs->MoveFirst;
	    while ( !$RScs->EOF ) {
    ...
    <tr>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Scientist Comments</font></strong></td>
      <td><input type="text" name="qScientistComments" size="20" value=""></td>
      <td valign="top"><strong><font face="Arial,Helvetica" size="-1">Data Comments</font></strong></td>
      <td><input type="text" name="qOtherComments" size="20" value=""></td>
    </tr>
  </table>
  <font face="Arial,Helvetica" size="-1">
  Return results as <input type=radio name="data_table" value="no" checked>cruise report list
  <input type=radio name="data_table" value="yes">data table
  </font>
  
  <br> 
  <input type="hidden" name="search" value="advanced"> 
  <input type="submit" name="Continue" value="Search for expeditions"> 
...
if ( GetFormValue('qDiveName') ) {
	GetFormValue('qDiveName') =~ /(^[tvdTVD]*)(\d+)$/;
...
#
# Construct where clause for ship name(s)
#
if ( GetFormValue('qShipName') ) {
	$ship_clause = "(ShipName = '" . GetFormValue('qShipName') . "')\n";
	##$ship_clause .= "AND (" if GetFormValue('search') eq 'advanced';
}
else {		# Look for both ships
	$ship_clause = "";
	##$ship_clause .= "AND (" if GetFormValue('search') eq 'advanced';
}
...
##$sql .= "(ShipName = '" . GetFormValue('qShipName') . "')\n" . 
##		"AND (" if GetFormValue('qShipName')&& GetFormValue('search') eq 'advanced';
...
##$sql .= "(" unless GetFormValue('qShipName');

#
# Add on the 'free-text' search criteria
#
$sql .= "(";
foreach $q ( @qFields ) {
	$f = $q; $f =~ s/^q//;
...
#
# remove trailing things from SQL
#
$sql =~ s/\nAND\s*$//;
$sql =~ s/\nOR\s*$//;
$sql .= ")";
...
if(!$RS) {
 	die "<BR>Empty Return Set: RS. for $count_sql ";
}
else {
...
if ( $num_expd > 100 &&  GetFormValue('count_check') !~ /yes/i ) { %>
	...
	<input type="button" name="count_check" value="No. Go back to search" onclick="history.go( -1 );return true;">
	</form>
	<form><%
	...
	<input type="submit" name="count_check" value="Yes. Show me all <%= $num_expd%> expeditions">
	</form>
	<br>
	(If you display this long list please wait for your browser to finish loading the page.)
	<%
	exit;
}
$RS->Close;
...
$RS = $Conn->Execute($sql);
if(!$RS) {
 	die "<BR>Empty Return Set: RS. for $sql ";
}
...
#
# Now cursor throught the record set and print the expeditions
#
if ($_[0] eq 'data_table') { %>
...
while ( !$RS->EOF ) {
	...
	elsif ( $RS->Fields('ShipName')->Value eq 'rcsn' ) {
		$RovName = 'vnta';
	}
	...
	if ( $_[0] eq 'data_table' ) { 
		...
		# Put a break after each data link for table display (easier to read)
...
if ( $_[0] eq 'data_table' ) { %>
	...
	<%
	if ( $has3Dlinks == 1 ) { %>
		<input type="submit" name="3D" value="Continue to select terrain for all above selected 3D Replays">
	<% } %>
	</form>
...
%>
<p><b>Total of <%= $count%> expeditions found with query:</b>
...
foreach $f ( @{$Application->{'ExpeditionFields'}} ) {
	...
	if ($f eq 'Accomplishments' || $f =~ /Comments$/ || $f eq 'Participants' ) {	# Standard text area
		...
		$ffff = "DataComments";
			...
			$cell .= "<textarea name=\"$f\" cols=\"60\" rows=\"3\" wrap>" . 
...
%><tr>
	<td valign="top"><font face="helvetica,Ariel" size="-1"><b><%= $ffff%>
	</b></font></td>
	<td valign="top"><font face="helvetica,Ariel" size="-1"><%= $cell%></font></td>
</tr>
	<%
}	# End foreach(
...
while ( !$RSdive->EOF ) {			# List existing dives in the database
	...
	$editlink .= "<img src=\"edit_this_dive.gif\" border=0></a>";
	##$editlink .= "<font face=\"helvetica,Ariel\" color=\"brown\" size=\"-2\"> to enter Start & End time, etc.</font>";
...
<% if ( $Session->{'RegisteredUser'} ) { %>	
	<font face="helvetica,Ariel" size="-1">Send postcruise updates email to 
	<input type="text" name="post_email" value="postcruise" size="8" maxlength="15">
	@mbari.org.</font></p>
	<br>
	<center>
	<input type="submit" name="finish" value="Update database with above changes and continue with dive information"> 
	<br><br>
	<input type="submit" name="finish" value="Done with this expedition. Update changes made above">
	<br><br>
...
%>
	...
	<br><br>
	<center>
	<input type="submit" name="finish" value="Continue with this expedition and enter new dive information">
	<br>
	<font face="helvetica,Ariel" size="-1">
	...
	<br><br>
	<input type="submit" name="finish" value="Done with this expedition. Submit to Logistics Coordinator">
	<br><br>
	<input type="reset" name="reset" value="Reset form fields">
	</center>
<%}
