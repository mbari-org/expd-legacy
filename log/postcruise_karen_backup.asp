...
<!--#include file="postcruise_hdr_karen.inc"-->
<!--#include file="expd_functions_karen.inc"-->

<% 
=head1 NAME

postcruise_karen.asp - Application for searching and entering post-cruise information

=head1 SYNOPSIS

    #http://expd.mbari.org/expd/log/postcruise_karen.asp
...
$this_script = 'postcruise_karen.asp';
...
if ( GetFormValue('step') eq '1' || ! GetFormValue('step') ) {
    ...
    %><h2>Edit Non-MBARI Dive <%= GetFormValue('RovName')%><%= GetFormValue('DiveNumber')%></h2>
    <font color="#408080"><strong>Fill in or edit the fields below, following
    the instructions in <span class="browntext">brown</span><font color="#408080">. Then submit
    the form to the Logistics Coordinator.</font></strong> </font>
    </td></tr></table><% 
	...
	%><h2 align="center">Step 4: Finished with Expedition <%= GetFormValue('ShipName')%><%= GetFormValue('ShipSeqNum')%></h2>
	</td></tr></table><%
...
<%
...
elsif (  GetFormValue('finish') =~ /dive information/i ) {		# Continue to enter dives using dive_karen.asp%>
	...
	<table width="680">
	<form method="get" action="dive_karen.asp">
	...
	<font size="+3">
	<input type="submit" class="button blue" name="add_dive" value="Continue to enter new dive information">
	</font>
...
function ShipSelect(select) {
    ...
    if (select.options[select.selectedIndex].value == "othr") {
        ...
        html:
          '<br> ' +
          '<b>For Non-MBARI ship cruises (e.g., miniROV), there are no ExpeditionIDs to search on.  As a result, certain MBARI ship-specific query variables are <br>Not Allowed (N/A).</b>',
	...
	if (select.options[select.selectedIndex].value == "wfly" || "rcsn" || "ptlo" || "zphr" || "prgn") {
		//console.log("MBARI-ship");
...
<form method="POST" action="<%= $this_script%>" id="form1" name="form1">
  ...
  <table table-layout="fixed" border="0" cellpadding="5" cellspacing="0">
    ...
    <tr>
      ...
      <td><div class="shipSelect"><select id="qShipName" name="qShipName" onChange="return ShipSelect(this)">
		<option value=""></option>		
		...
		<option value="othr">Non-MBARI Ship - MiniROV</option>		
		</select></div>
      </td>
    ...
    <tr>
      ...
      <td><input type="text" name="qDiveName" size="6">
      <span class="smallbrownText">May precede with first ROV Name Letter</span><br>
      <span class="smallbrownText">&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;(e.g., V3296, T1001, D1, M32)</span>
      </td>
...
#
if ($_[0] eq 'data_table') { %>
	<form action="/3Dreplay/3D_karen.asp" target="3Dreplay">
	<table BORDER="1">
	<tr>
...
while ( !$RS->EOF ) {
	...
	#
	if ( $RS->Fields('ShipName')->Value eq 'ptlo' ) {
		$RovName = 'vnta';
	}
	elsif ( $RS->Fields('ShipName')->Value eq 'rcsn' && $qRovName ne 'mini') {
		$RovName = 'vnta';
	}
	elsif ( $RS->Fields('ShipName')->Value eq 'wfly' && $yr < 2008) {
		$RovName = 'tibr';
	}
	elsif ( $RS->Fields('ShipName')->Value eq 'wfly' && $yr > 2008) {
		$RovName = 'docr';
	}
	elsif ( $RS->Fields('ShipName')->Value eq 'othr' && $RS->Fields('DeviceID')->Value eq 1859 || $qRovName eq 'mini') {
 		$RovName = 'mini';
 	}
	elsif ( $RS->Fields('ShipName')->Value eq 'prgn') {
		$RovName = '';
	}
	$sql = "SELECT * FROM Dive ";
...
#####print "<br>1.. $sql<br";
    ...
    if( $RSdive->Fields('DiveNumber')->Value > 0 ) {
	 	$dive_text = '<BR>Edit Dives: ';
	 	while ( !$RSdive->EOF ) {
			
			##$dive_text .= "<br>" if ($_[0] eq 'data_table');
			$dive_text .= "<a href=\"dive_karen.asp?RovName=" . $RSdive->Fields('RovName')->Value;
			$dive_text .= "&amp;DiveNumber=" . $RSdive->Fields('DiveNumber')->Value . "&amp;edit=yes\" target=\"diveedit\">";
			$dive_text .= $RSdive->Fields('RovName')->Value;
			
			if ( $RSdive->Fields('DiveNumber')->Value == GetFormValue('qString') || $RSdive->Fields('DiveNumber')->Value == $qDiveNumber ) {
	...
	if ($Status eq 'Completed') {
		...
		$StatusString .= " &middot; <a href=precruise_karen.asp?step=revise&ShipName=" . $RS->Fields('ShipName')->Value;
	...
	if ( $RovName ) {
		my $rdbdlinks = dataLinks( ExpeditionID_FK => $ExpeditionID, DataType => 'rovctddb', SearchDiveNumber => $qDiveNumber );
		my $proclinks = dataLinks( ExpeditionID_FK => $ExpeditionID, DataType => 'rovctd' );

    	# Potential links to Non-MBARI SHIPS OF OPPORTUNITY
    	my $rdbdMinilinks = dataLinks ( ExpeditionID_FK => 0, DataType => 'rovctddb', SearchDiveNumber => $qDiveNumber );
    	my $procMinilinks = dataLinks( ExpeditionID_FK => 0, DataType => 'rovctd' );

		if ( $rdbdlinks ) {
			$expd_text .= "\n<br><font face=\"helvetica,Ariel\"><b>ROVCTD Data:</b> $rdbdlinks</font>";
		}
    	elsif ( $rdbdMinilinks ) {
      		$expd_text .= "\n<br><font face=\"helvetica,Ariel\"><b>ROVCTD Data (non-MBARI ship):</b> $rdbdMinilinks</font>";
    	}
		elsif ( $proclinks ) {
			$expd_text .= "\n<br><font face=\"helvetica,Ariel\" color=\"#c0c0c0\"><b>ROVCTD Data:</b></font> ";
			$expd_text .= "<font color=\"brown\">";
			$expd_text .= "<A HREF=\"dive_karen.asp?ExpeditionID=$ExpeditionID&add_dive=Continue+to+enter+new+dive+information\" target=\"dive_edit\">";
			$expd_text .= "Add dive(s)</A> to enter dive number(s), start and end times so that additional data products may be produced. (<i>This message is replaced by links after dive(s) have been entered and nightly job has run.</i>)</font>";
      		...
      		$expd_text .= "\n<br><font face=\"helvetica,Ariel\" color=\"#c0c0c0\"><b>ROVCTD Data (non-MBARI ship):</b></font> ";
      		$expd_text .= "<font color=\"brown\">";
      		$expd_text .= "<A HREF=\"dive_karen.asp?ExpeditionID=0&add_dive=Continue+to+enter+new+dive+information\" target=\"dive_edit\">";
      		$expd_text .= "Add dive(s)</A> to enter dive number(s), start and end times so that additional data products may be produced. (<i>This message is replaced by links after dive(s) have been entered and nightly job has run.</i>)</font>";
    	}
	
		$rdlinks = dataLinks( ExpeditionID_FK => $ExpeditionID, DataType => 'rovctd', SearchDiveNumber => $qDiveNumber );
		$expd_text .= "\n<br><font face=\"helvetica,Ariel\"><b>ROVCTD Processing:</b> <font size=\"-2\">$rdlinks</font></font>" if $rdlinks;

    	# Potential search for links on Non-MBARI Ships of Opportunity
    	$rdMinilinks = dataLinks( ExpeditionID_FK => 0, DataType => 'rovctd', SearchDiveNumber => $qDiveNumber );
    	$expd_text .= "\n<br><font face=\"helvetica,Ariel\"><b>Mini ROVCTD Processing:</b> <font size=\"-2\">$rdMinilinks</font></font>" if $rdMinilinks;
	}
	...
	##if ( $brows =~ /mac/i ) {
	##	$d3links = "<font color=\"brown\">3D Replay currently works in browsers on Windows computers</font>";
	##}
	
	##$expd_text .= "\n<br><font face=\"helvetica,Ariel\"><b>3D Replay:</b> $d3links</font>" if $d3links_orig =~ /3D.asp/;
    ...
    $rdMinilinks = '&nbsp;' unless $rdMinilinks;
		...
		if ( ($mydive_text eq '' || $mydive_text =~ /No/) && $Status !~ /Canc/i && $Status !~ /Aborted/i 
		  ...
		  $mydive_text .= "<br>";
		  $mydive_text .= "<A HREF=\"dive_karen.asp?ExpeditionID=" . $RS->Fields('ExpeditionID')->Value . 
    	...
    	$rdMinilinks =~ s/&middot;/&middot;<br>/g;
		$rdbdlinks =~ s/&middot;/&middot;<br>/g;
    	$rdbdMinilinks =~ s/&middot;/&middot;<br>/g;
		...
		##$d3links = "<form action=\"/3Dreplay/3D_karen.asp\">" . $d3links;
		...
		#
		##my $brows = $Request->ServerVariables(HTTP_USER_AGENT)->{Item};
		##if ( ($brows =~ /mac/i) && $d3links_orig =~ /3D_karen.asp/ ) {
    ...
    <td valign="top"><font face="helvetica,Ariel" size="-2"><%=$rdbdMinilinks%><%=$rdMinilinks%></font></td>    
...
<a href="/samplesDB/queries/default_karen.asp">Query the Samples Database</a>
<br>
...
if ($qRovName eq 'mini') {
    ...
    please <br>consider revising your <a href="https://mww.mbari.org/expd/log/postcruise_karen.asp?search=advanced">EXPD search(es)</a> to include both MBARI and Non-MBARI vessels.<br>
...
foreach $f ( @{$Application->{'ExpeditionFields'}} ) {
	...
	if ($f eq 'Accomplishments' || $f =~ /Comments$/ || $f eq 'Participants' ) {	# Standard text area
		...
		$cell .= $cs_option_list;
		$cell .= "</select>\n";
		$cell .= "<a href=person_karen.asp>Edit person list</a>";
	}
...
%><tr>
	...
	$dlinks .= "<b>&middot;</b>" . $rawNavdbLinks if $rawNavdbLinks;
	my $cleanNavdbLinks = dataLinks( ExpeditionID_FK => $ExpeditionValue{ExpeditionID}, DataType => 'CleanNavdb' );
	$dlinks .= "<b>&middot;</b>" . $cleanNavdbLinks if $cleanNavdbLinks;
...
while ( !$RSdive->EOF ) {			# List existing dives in the database
  ...
  # SEND THIS OVER TO DIVE.ASP
	$editlink .= "<a href=dive_karen.asp?RovName=" . $RSdive->Fields('RovName')->Value . "\&";
	$editlink .= "DiveNumber=" . $RSdive->Fields('DiveNumber')->Value . "&edit=yes target=\"diveedit\">"; 
...
Present Non-MBARI Ship / MiniROV dive info for editing.
...
Return list of cruises from query specified in step1() or advanced_search()
SPECIFICALLY For NON-MBARI SHIPS (miniROV) searches.
If argument of 'data_table' passed then return results as a table.
...
################################
    ...
    if( $RSdive->Fields('DiveNumber')->Value > 0 ) {
        ...
        # Loop through the Dives (only one miniROV dive per DiveID - No ExpeditionID here.)
	 	for ($d=0; $d<1; $d++) {
			$dive_text .= "<a href=\"dive_karen.asp?RovName=" . $RovName;
			$dive_text .= "&amp;DiveNumber=" . $DiveNumber . "&amp;edit=yes\" target=\"diveedit\">";
			$dive_text .= $RSdive->Fields('RovName')->Value;
		...
		$dive_text =~ s/$delimiter$//;
		$dive_text .= " <font color=brown>(These links also show CTD summary information and timeline of framegrabs and coolpix images)</font><br>";
	}
	else {
		if ($Status eq 'Completed') {
			$dive_text = "<br><br><font color=brown>No dives entered in database</font>";
    ...
    $StatusString .= "Non-MBARI Ship </strong></font> &middot; ";
      ...
      $StatusString .= " &#150; $EndDate - $endhrmn &middot; $endYYYYDDD <em>&bull;</em> " ;
		...
		$StatusString .= " &middot; <a href=precruise_karen.asp?step=revise&RovName=" . $RovName;
	...
	if ( $RovName ) {
    	# Potential links to Non-MBARI SHIPS OF OPPORTUNITY
    	my $rdbdlinks = dataLinks ( ExpeditionID_FK => $ExpeditionID, RovName_FK => $RovName, DataType => 'rovctddb', SearchDiveNumber => $qDiveNumber );
    	my $proclinks = dataLinks( ExpeditionID_FK => $ExpeditionID_FK, RovName_FK => $RovName, DataType => 'rovctd', SearchDiveNumber => $qDiveNumber );

		if ( $rdbdlinks ) {
      		$expd_text .= "\n<br><font face=\"helvetica,Ariel\"><b>ROVCTD Data (non-MBARI ship):</b> $rdbdlinks</font>";
    	}
		elsif ( $proclinks ) {
      		$expd_text .= "\n<br><font face=\"helvetica,Ariel\" color=\"#c0c0c0\"><b>ROVCTD Data (non-MBARI ship):</b></font> ";
      		$expd_text .= "<font color=\"brown\">";
		...
		# Potential search for links on Non-MBARI Ships of Opportunity
    	$rdlinks = dataLinks( ExpeditionID_FK => $ExpeditionID_FK, DataType => 'rovctd', SearchDiveNumber => $qDiveNumber );
    	$expd_text .= "\n<br><font face=\"helvetica,Ariel\"><b>Mini ROVCTD Processing:</b> <font size=\"-2\">$rdMinilinks</font></font>" if $rdMinilinks;
	}
    ...
    $rdMinilinks = '&nbsp;' unless $rdMinilinks;
		...
		if ( ($mydive_text eq '' || $mydive_text =~ /No/) && $Status !~ /Canc/i && $Status !~ /Aborted/i 
		  ...
		  $mydive_text .= "<br>";
		  $mydive_text .= "<A HREF=\"dive_karen.asp?ExpeditionID=" . $RSdive->Fields('ExpeditionID_FK')->Value . 
						"&add_dive=Continue+to+enter+new+dive+information\" target=\"dive_edit\">" .
						 "<img src=\"add_dives.gif\" alt=\"Add dive(s)\" border=\"0\"></A>";
		}
		$ElinkString .= $mydive_text;
    ...
    <%if ( GetFormValue('qShipname') eq 'othr' ) {
      ...
      <a href="<%= $this_script%>?step=5&amp;RovName=<%= $RS->Fields('RovName')%>&amp;DiveNumber=<%= $RS->Fields('DiveNumber')%>&amp;edit=<%= $diveName%>" target="edit"><img src="edit_post.gif" border="0" WIDTH="114" HEIGHT="21"></a>
      <font face="helvetica,Ariel" size="-1">
...
while ( !$RSdive->EOF ) {
	foreach $f ( @{$Application->{'DiveFields'}} ) {
		...
		if ( $f eq 'DiveStartDtg' ) {
			my $link = "dive_karen.asp?DiveNumber=" . $RSdive->Fields(DiveNumber)->Value;
...
sub SelectEpoch {
	...
	if ($ExpdID == 0) {
		my $time_field = $_[0];
		#print "<br>SelectEpoch Expd=$ExpdID<br>";
	...
	my $time_field = $_[0];
		#print "<br>SelectEpoch Expd!=0<br>";
