...
<!--#include file="precruise_hdr_karen.inc"-->
...
<a href="region_karen.asp?add=new">click here to add a region to the list</a>.</b>
<p>
...
<% while ( !$RS->EOF ) { %>
	...
	<% foreach $f (@fields) { 
		...
		if ($f eq 'RegionID') {
			$val = "<a href=\"region_karen.asp?edit=$val\">$val</a>";
		} %><%= $cell%><%= $val%></td>
	<% } %>
	<% $RS->MoveNext; %>
<% } %>
...
<form method="get" action="region_karen.asp">
...
<p><a href="precruise_karen.asp?step=1">Return to Pre-Cruise entry form step 1</a></p>
<p><a href="region_karen.asp">Return to Region List</a></p>
