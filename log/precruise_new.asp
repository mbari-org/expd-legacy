...
=head1 SYNOPSIS
    ...
    http://expd.mbari.org/expd/log/precruise_rcsn.asp
...
<%
$ship_code = GetFormValue('ShipName');
if ($ship_code eq 'rcsn') {
	$ship_display_name = 'Rachel Carson';
}
elsif ($ship_code eq 'wfly') {
	$ship_display_name = 'Western Flyer';
}
elsif ($ship_code eq 'prgn') {
	$ship_display_name = 'Paragon';
}
else {
	$ship_display_name = '<font color="red">Unknown Ship</font>';
}
%>
		<input type="hidden" name="ShipName" value="<%=$ship_code%>">
        <tr>
            <td valign="top">
            <strong><font face="Arial,Helvetica">Ship Name</font></strong>
			</td>
            <td valign="top">
            <strong>R/V <i><%=$ship_display_name%></i></strong>
			</td>
        </tr>
        <tr>
            <td valign="top">
