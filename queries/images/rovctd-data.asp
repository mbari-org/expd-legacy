...
' since the each row of data is related to the next, and there are an arbitrary number of
...
'
' http://fog.shore.mbari.org/expd/queries/rovctd-data.asp?
' year=2000&
' yday=234&
' dive=1024&
' date_type=srt_end&
...
'2. pltfrm (vnta,tibr) - The value indicates which ROV was used, Ventana
'   or Tiburon.
...
Dim varPltfrm			' holds the platform (tibr/vnta)
Dim varROVName			' holds the rov name (Tiburon/Ventana)
        ...
        Case "dive"
            ...
            Else
                ...
                if divewarning = 1 then
                    ...
                    Response.Write ("<A HREF=" & Chr(34) & "../log/dive.asp?RovName=" & Request.QueryString("pltfrm") & _
...
if (quitnow = 0) then
    ...
    If (rstRovctd.BOF and rstRovctd.EOF) then
        ...
        Call WriteHTMLFooter ()
    Else
        '***** This is the first conntent written to the user's browser (except for errors) *****'
