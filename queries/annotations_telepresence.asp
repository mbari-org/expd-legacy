...
<body bgcolor="#FFFFFF">
 ...
 ' (add new ship names if others are ever in VARS database)
  ...
  strSQL = strSQL & " FROM Annotations_legacy NOLOCK "
  strSQL = strSQL & " WHERE "
  ...
  <b>&#183;</b> 
  <a href="VARS2staging_telepresence.asp?RovName=<%=strVarsROVName%>&qSCollDTG=<%=startCollDTG%>&qECollDTG=<%=endCollDTG%>&qDiveNameNum=<%=strDiveNameNum%>">Select samples for loading into the Samples database</a>
  <br>
         ...
         ' Create a string for all association values
		  ...
		  If Not IsNull(strObservationID) Then
			  ...
			  'strSQL2 = "SELECT LinkName, ToConcept, LinkValue FROM Associations_legacy " & _
				''		  "WHERE (ObservationID_FK = '" & strObservationID & "')"
        strSQL2 = "SELECT link_name AS LinkName, to_concept AS ToConcept, link_value AS LinkValue FROM associations " & _
              "WHERE (observation_uuid = '" & strObservationID & "')"
				%>
				  <!-- Exectuing strSQL2:
				  <%=strSQL2%>
				  -->
