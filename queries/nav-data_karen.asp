...
=head1 SYNOPSIS
    ...
    > http://expd.mbari.org/expd/queries/nav-data_karen.asp?ROVName=vnta&DiveNumber=2606
    > http://expd.mbari.org/expd/queries/nav-data_karen.asp?ROVName=vnta&DiveNumber=2606&type=raw
    > http://expd.mbari.org/expd/queries/nav-data_karen.asp?ROVName=vnta&Esecs=1203443481
...
##$_dsn = "Server=fog;Database=expd_ks;UID=***;******;";
...
sub returnRecords {
	...
	%>
		<h2>'<%=$rov%> is not a valid ROVName.  Please choose 'vnta', 'tibr' or 'mini'</h2>
	<%
		return;
	}
	...
	# Default is to return CleanNav, if 'type=raw' is specified then return from RanNav view
	if ( $rawFlag =~ /raw/i ) {
		$view .= "RawNavData";
	}
	else {
